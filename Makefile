#SET THE NAME OF THE PROBLEM FILE - E.G. "wave2.c" CORRESPONDS
#WITH OUTPUT=wave2
OUTPUT = wavewind


#SET FILES OF INTEREST 
SRC_FILES = $(addsuffix .c,$(OUTPUT))
OBJ_FILES = $(addsuffix .o,$(basename $(SRC_FILES)))
#IF YOU WANT TO SET A FOLDER THAT THE EXECUTABLE GOES INTO:
#OUT_FOLDER = $(addprefix f,$(OUTPUT))
#OTHERWISE, JUST SET THE CURRENT DIRECTORY
OUT_FOLDER = .

#SET C FLAGS AND BASILISK COMPILER, LINKING FLAGS
CFLAGS += -Wall -O2 #-fopenmp
BAS=qcc
LDFLAGS = -lm

#SET FLAGS FOR THE GRAPHICS
OPENGLIBS = -L${BASILISK}/gl -lfb_osmesa -lOSMesa -lglut -lGLU
OPENGLINC = ${BASILISK}

#MAKE COMMAND: TAKES ARGUMENT AND MAKES EXECUTABLE IN DESIRED FOLDER
$(OUTPUT): 
	$(BAS) $(CFLAGS) $(SRC_FILES) -I$(OPENGLINC) -o $(OUT_FOLDER)/$(OUTPUT) $(OPENGLIBS) $(LDFLAGS)
