...
#line 218 "wavewind.c"
 ...
 _attribute[psi.i].boundary[top] = _boundary5; _attribute[psi.i].boundary_homogeneous[top] = _boundary5_homogeneous;
 _attribute[psi.i].boundary[bottom] = _boundary6; _attribute[psi.i].boundary_homogeneous[bottom] = _boundary6_homogeneous;
...
#line 247 "wavewind.c"
{
 ...
 end_trace("init_0", "wavewind.c", 268); } return 0; } 
#line 281 "wavewind.c"
...
static int graphs_expr0 (int * ip, double * tp, Event * _ev) {  int i = *ip; double t = *tp;  int ret = (i++);   *ip = i; *tp = t;   return ret; } static int graphs (const int i, const double t, Event * _ev) { trace ("graphs", "wavewind.c", 320);  {
...
#line 334
  ...
  fprintf (ferr, "%g %g %g %g\n",
    t/(k_/sqrt(g_*k_)), ke/2., gpe + 0.125, dissWater);
 end_trace("graphs", "wavewind.c", 349); } return 0; } 
#line 364 "wavewind.c"
static int movies_expr0 (int * ip, double * tp, Event * _ev) {  int i = *ip; double t = *tp;  int ret = (t += 0.1);   *ip = i; *tp = t;   return ret; } static int movies (const int i, const double t, Event * _ev) { trace ("movies", "wavewind.c", 364);  {
...
#line 455 "wavewind.c"
 ...
 delete (((scalar []){omega,{-1}}));  end_trace("movies", "wavewind.c", 459); } return 0; } 
...
static int snapshot_expr0 (int * ip, double * tp, Event * _ev) {  int i = *ip; double t = *tp;  int ret = (i += 200);   *ip = i; *tp = t;   return ret; } static int snapshot (const int i, const double t, Event * _ev) { trace ("snapshot", "wavewind.c", 467);  {
  dump ((struct Dump){"dump"});
 end_trace("snapshot", "wavewind.c", 469); } return 0; } 
...
static int end_expr0 (int * ip, double * tp, Event * _ev) {  int i = *ip; double t = *tp;  int ret = (t = 40.*k_/sqrt(g_*k_));   *ip = i; *tp = t;   return ret; } static int end (const int i, const double t, Event * _ev) { trace ("end", "wavewind.c", 477);  {
  fprintf (fout, "i = %d t = %g\n", i, t);
  dump ((struct Dump){"end"});
 end_trace("end", "wavewind.c", 480); } return 0; } 





static int dumpstep_expr0 (int * ip, double * tp, Event * _ev) {  int i = *ip; double t = *tp;  int ret = (t += k_/sqrt(g_*k_)/32);   *ip = i; *tp = t;   return ret; } static int dumpstep (const int i, const double t, Event * _ev) { trace ("dumpstep", "wavewind.c", 486);  {
  char dname[100];
  sprintf (dname, "dump%g", t/(k_/sqrt(g_*k_)));
  dump ((struct Dump){dname});
 end_trace("dumpstep", "wavewind.c", 490); } return 0; } 
#line 499 "wavewind.c"
static int adapt_0_expr0 (int * ip, double * tp, Event * _ev) {  int i = *ip; double t = *tp;  int ret = (i++);   *ip = i; *tp = t;   return ret; } static int adapt_0 (const int i, const double t, Event * _ev) { trace ("adapt_0", "wavewind.c", 499);  {
  adapt_wavelet ((struct Adapt){((scalar []){f,{-1}}), (double[]){femax,uemax,uemax,uemax}, LEVEL, 5});
 end_trace("adapt_0", "wavewind.c", 501); } return 0; } 
#line 79 "/home/jiarongw/basilisk/src/navier-stokes/centered.h"
