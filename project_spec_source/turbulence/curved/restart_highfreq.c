...
double TRESTART = 0; //Use to restart dump files and have higher frequency output 
...
event init (i = 0) {
  /* char dumpname[100]; */
  /* sprintf (dumpname, "dump%g", TRESTART); */
...
void output_twophase (double snapshot_time) {
  ...
  position (f, pos, G, Z);
  char etaname[100];
  sprintf (etaname, "./eta_highfreq/eta_t%g_%d", snapshot_time, pid());
...
void output_twophase_locate (double snapshot_time) {
  ...
  position (f, pos, G, Z);
  char etaname[100];
  sprintf (etaname, "./eta_highfreq/eta_loc_t%g_%d", snapshot_time, pid());
  FILE * feta = fopen (etaname, "w");
...
event p_stat (t += 0.05) {
  ...
  for (int i=0; i<Nslice; i++) {
    zslice += L0/Nslice;
    sprintf (filename, "./field_highfreq/pair_run_t%g_slice%d", t, i);
    sliceXY (filename,pair,zslice,9);
    sprintf (filename, "./field_highfreq/f_t%g_slice%d", t, i);
