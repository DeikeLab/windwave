...
void movies () {
  ...
  clear();
  view (fov = 25, camera = "front", ty = -0.5, width = 600, height = 600, bg = {1,1,1}, samples = 4);
  squares ("uwater", linear = false, n = {0,0,1}, alpha = -0.1, map = cool_warm);
  ...
  {
    FILE * fp = fopen ("uwater.ppm", "a");
    save (fp = fp);
  }
  clear();
  view (fov = 25, camera = "front", ty = -0.5, width = 600, height = 600, bg = {1,1,1}, samples = 4);
  squares ("level", linear = false, n = {0,0,1}, alpha = -0.1, max = 10, min = 5);
  //cells (n = {0,0,1}, alpha = -0.1);
  //draw_vof ("f", color = "u.x");
  draw_string (s, size = 30);
  {
    FILE * fp = fopen ("level.ppm", "a");
    save (fp = fp);
  }
}
