...
event init (i = 0) {
  ...
  refine ((y < 0.01 || y > 0.99) && level < (MAXLEVEL+3));
}
...
/* #endif */
  ...
  {
    sprintf (filename, "./movie/Ux%d.png", frame);
    FILE * fp = fopen(filename, "w");
    save (fp = fp);
  }
  squares ("u.y", linear = true, n = {0,0,1}, alpha = 0);
  {
    sprintf (filename, "./movie/Uy%d.png", frame);
    FILE * fp = fopen(filename, "w");
    save (fp = fp);
  }
  squares ("u.z", linear = true, n = {1,0,0}, alpha = 0);
  {
    sprintf (filename, "./movie/Uz%d.png", frame);
    FILE * fp = fopen(filename, "w");
    save (fp = fp);
  }
  squares ("omega", linear = true, n = {0,0,1}, alpha = 0);
  {
    sprintf (filename, "./movie/omega%d.png", frame);
    FILE * fp = fopen(filename, "w");
    save (fp = fp);
  }
  ...
  isosurface ("l2", -1);
  {
    sprintf (filename, "./movie/vortex%d.png", frame);
    FILE * fp = fopen(filename, "w");
    save (fp = fp);
  }
  frame++;
